...
Además, se introducen modificaciones como un sistema de recompensas basado en el progreso hacia la meta y la colocación de barreras. Estas reglas adaptadas permiten evaluar de forma más granular las estrategias y decisiones tomadas por los agentes.
...
El valor de Alpha = 0.2 ha demostrado ser más efectivo que Alpha = 0.1, permitiendo una mejor adaptación y alcanzando valores positivos de Avg_Score en 5000 episodios. Este hallazgo indica que una tasa de aprendizaje ligeramente más alta facilita una actualización más rápida de la función de valor, lo que contribuye a una exploración más eficiente del espacio de estados. Sin embargo, la baja tasa de victorias sugiere que la función de recompensa puede estar incentivando estrategias que optimizan la puntuación sin priorizar la victoria. Es posible que el agente esté aprendiendo patrones que maximizan las recompensas inmediatas sin desarrollar un enfoque global que incremente las posibilidades de ganar la partida.

Otro aspecto relevante es el uso de barreras, que varía a lo largo del entrenamiento y sugiere que el agente modifica su estrategia con el tiempo. En las etapas iniciales, el agente parece centrarse en movimientos directos hacia la meta, mientras que en fases más avanzadas comienza a incorporar el uso de barreras como una forma de obstaculizar al oponente. Este comportamiento emergente indica que el agente está explorando diversas estrategias y adaptándose a las circunstancias del juego. Esto plantea la posibilidad de que, con más episodios o un refinamiento en la política de exploración, el agente pueda desarrollar tácticas aún más sofisticadas que le permitan equilibrar la optimización de la puntuación y la probabilidad de ganar.

Dado que el diseño actual de la función de recompensa podría estar influyendo en estos resultados, futuros experimentos podrían centrarse en ajustar estos parámetros. Modificaciones en la forma en que se recompensa el uso de barreras o el acercamiento a la victoria podrían ayudar a alinear mejor el aprendizaje del agente con el objetivo final de ganar partidas. Además, la implementación de técnicas como la adaptación dinámica del valor de Alpha o la incorporación de mecanismos de exploración más avanzados, como épsilon-decay ajustado, podría mejorar la capacidad del agente para equilibrar la exploración y la explotación.

En conclusión, aunque el agente ha mostrado progresos significativos en la optimización de su desempeño dentro del entorno de Quoridor, persisten desafíos relacionados con la traducción de estos avances en una mayor tasa de victorias. Ajustes más precisos en la función de recompensa, combinados con estrategias de exploración más refinadas, podrían permitir que el agente desarrolle un enfoque más integral y efectivo para competir en el juego. Este trabajo sienta las bases para futuras investigaciones orientadas a mejorar la capacidad de aprendizaje y adaptación de los agentes inteligentes en entornos complejos y dinámicos.
